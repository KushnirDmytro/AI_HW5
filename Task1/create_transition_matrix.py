...
class Move:
    def __init__(self,  dir_name, dirs_and_probabilities_dict):
...
class Field:
    ...
    def position_is_possible(self, final_position):
        if 0 <= final_position[0] < self.cols and 0 <= final_position[1] < self.lines:  # TODO make blocks    ==> and final_position not in self.
            return True
        else:
            return False
    ...
    def fill_transition_matrix(self):
        for move_indx in range(len(self.moves)): #switching over moves
            #TODO fill rest of dimations
            this_move_name = self.moves[move_indx].dir_name #TODO delete it from structure
            ...
            for y in range(self.lines):
                for x in range(self.cols):
                    ...
                    for transition_name in self.moves[move_indx].moves_dict:
                        ...
                        final_position_indx = self.index_from_coords(x=final_position_coords[0], y=final_position_coords[1])
                        ...
                        if self.position_is_possible(final_position_coords):
                            ...
                            self.np_matrix[move_indx][start_position_indx][start_position_indx] += exactly_this_transition_prob
    ...
    def pretty_print(self):
        for m in range(len(self.moves)):
            print(" FOR MOVE " + str(self.moves[m].dir_name))
...
move_top = Move(dir_name='top',
                dirs_and_probabilities_dict = {'top': 0.8, 'bot': 0.0 ,'left': 0.1,'right': 0.1})
move_right = Move(dir_name='right',
                dirs_and_probabilities_dict = {'top': 0.1, 'bot': 0.1 ,'left': 0.0,'right': 0.8})
move_bot = Move(dir_name='bot',
                dirs_and_probabilities_dict = {'top': 0.0, 'bot': 0.8 ,'left': 0.1,'right': 0.1})
move_left = Move(dir_name='left',
                dirs_and_probabilities_dict = {'top': 0.1, 'bot': 0.1 ,'left': 0.8,'right': 0.0})
